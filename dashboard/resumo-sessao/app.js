...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function abrirTelaResumo(patient) {
    ...
    window.location.href = `./resumindo/index.html?${query.toString()}`;
  }

  async function sairDoSistema() {
    await registrarEvento({
      evento: "logout",
      pagina: "resumo_sessao",
      perfil: "profissional",
      ...
      .map((patient) => {
        ...
        return `
          <button
            ...
            aria-label="Selecionar ${escapeHtml(patient.alias)} para resumo da sessão"
          >
  ...
  if (patientsGrid) {
    patientsGrid.addEventListener("click", (event) => {
      ...
      registrarEvento({
        ...
        email: currentProfile?.email || currentUser?.email || "",
        perfil: "profissional",
        evento: "paciente_selecionado_para_resumo_sessao",
        pagina: "resumo_sessao",
        contexto: {
  ...
  iniciar().catch((error) => {
    console.error("Erro na tela resumo-sessao:", error);
